...
def get_measures():
    ...
    if response.status_code == 200:
        ...
        unique_reported_measure_codes = datasets[['ReportedMeasureCode', 'ReportedMeasureName']].drop_duplicates()
...
def insert_into_postgresql(unique_measures):
    ...
    CREATE TABLE IF NOT EXISTS reportedmeasurements (
        "ReportedMeasureCode" TEXT PRIMARY KEY,
    ...
    try:
        ...
        for row in unique_measures.itertuples(index=False):
            cursor.execute(
                """
                INSERT INTO reportedmeasurements ("ReportedMeasureCode", "ReportedMeasureName")
                VALUES (%s, %s)
                ON CONFLICT ("ReportedMeasureCode") DO NOTHING
                """, 
                (row.ReportedMeasureCode, row.ReportedMeasureName)
            )
...
def callback(ch, method, properties, body):
    try:
        ...
        sdf = sdf.toPandas()
        sdf['ReportedMeasureName'] = sdf['ReportedMeasureName'].str.strip()
        sdf['ReportedMeasureCode'] = sdf['ReportedMeasureCode'].str.strip()
        unique_measures = sdf[['ReportedMeasureCode', 'ReportedMeasureName']].drop_duplicates()
